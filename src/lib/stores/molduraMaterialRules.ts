...
export interface MolduraExcludeRule {
  id: string;
  keyword: string;
  normalized: string;
  updated_at: string;
...
const STORAGE_KEY = 'moldura-material-rules-exclude';

// Seeds: Tapacanto + Círculos como ejemplos (migran el hardcode),
// más los falsos positivos reales reportados (tapiz, corte de tela).
const SEEDS = ['tapacanto', 'circulo', 'circular', 'tapiz', 'corte de tela'];

let cache: MolduraExcludeRule[] = [];
let loaded = false;
...
function toRule(keyword: string): MolduraExcludeRule {
...
function readLocal(): MolduraExcludeRule[] {
  try {
...
export async function load(): Promise<number> {
  ...
  try {
    const remote = await api.getMolduraMaterialRules();
    if (Array.isArray(remote) && remote.length > 0) {
      const mapped: MolduraExcludeRule[] = remote
...
export function getAll(): MolduraExcludeRule[] {
...
export function isExcluded(desc: string, rules?: string[] | MolduraExcludeRule[]): boolean {
...
export async function add(keyword: string): Promise<MolduraExcludeRule | null> {
  await ensureLoaded();
  const kw = keyword.trim();
  if (!kw) return null;
  ...
  persistLocal();
  try {
    await api.saveMolduraMaterialRule({ keyword: rule.keyword });
  } catch {
...
export async function remove(id: string): Promise<boolean> {
  ...
  if (Number.isInteger(numericId)) {
    try {
      await api.deleteMolduraMaterialRule(numericId);
    } catch {
      // no fatal
    }
  }
